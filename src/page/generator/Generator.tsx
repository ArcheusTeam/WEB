      ...
      {!selectedTone ? (
        ...
        <div className="space-y-8">
          ...
          <div className="p-4 bg-white rounded-lg shadow">
            <h2 className="text-xl font-bold mb-2">Template pour {selectedTone}</h2>
            <p>{tonePrompts[selectedTone]}</p>
            <p className="mt-2 italic text-gray-600">#{selectedTone} {toneConfig[selectedTone].emoji}</p>
          </div>
          ...
          {/* Résultat */}
          {post && (
            <div className="p-6 bg-white rounded-lg shadow-lg mt-6 border">
              <h2 className="text-lg font-semibold mb-2">🧵 Publication générée</h2>
              <p className="mb-4">{post.text}</p>
              <img src={post.gifUrl} alt="Image générée" className="rounded-md max-h-64 object-contain" />
              <div className="mt-4 text-2xl">{post.emoji}</div>
            </div>
          )}
        </div>
      )}
